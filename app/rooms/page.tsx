...
function RoomsContent() {
  ...
  // ─── NEW: SAFE BOOKING CHECK EFFECT ──────────────────────────
  useEffect(() => {
    async function fetchBookings() {
      if (!checkIn || !checkOut) return;
      try {
        const res = await fetch(`${STRAPI_URL}/api/bookings?populate=room&filters[booking_status][$in]=Pending,Approved`);
        if (!res.ok) return;
        ...
        const ids = bookings
          ...
          .map((booking: any) => booking.room?.documentId || booking.room?.id);
  ...
  return (
    <div style={{ minHeight: '100vh', color: '#1A1A1A', paddingTop: isMobile ? '5rem' : '6.5rem', boxSizing: 'border-box' }}>
      <style>{`
        ...
        .section-tag { color: #C8A87C; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.15em; font-weight: 600; display: inline-block; margin-bottom: 0.5rem; }
        .section-title { font-size: 2.5rem; font-weight: 700; color: #1A1A1A; line-height: 1.2; }
        @media (min-width: 768px) { .section-title { font-size: 3rem; } }
        @media (min-width: 1024px) { .section-title { font-size: 3.5rem; } }
        .section-divider { width: 4rem; height: 0.25rem; background: linear-gradient(90deg, #C8A87C 0%, #E8D5B8 100%); border-radius: 9999px; margin: 1rem auto 0; }
        .room-number-badge { position: absolute; top: 1rem; right: 1rem; background: rgba(0,0,0,0.6); backdrop-filter: blur(4px); color: #C8A87C; font-size: 0.85rem; font-weight: 700; padding: 0.3rem 0.9rem; border-radius: 9999px; border: 1px solid rgba(200,168,124,0.3); letter-spacing: 0.05em; }
        .room-card { transition: all 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94); background: #F8F8F8; border: 1px solid #E8E8E8; overflow: hidden; display: flex; flex-direction: column; }
        .room-card:hover { transform: translateY(-6px); box-shadow: 0 20px 60px rgba(0,0,0,0.08); border-color: #C8A87C; }
        .room-card .image-wrapper { position: relative; overflow: hidden; background: #F0F0F0; min-height: 280px; }
        .room-card .image-wrapper img { width: 100%; height: 100%; min-height: 280px; object-fit: cover; transition: transform 0.7s ease; }
        .room-card:hover .image-wrapper img { transform: scale(1.06); }
        .room-card .content { padding: 1.75rem 2rem 2rem; background: #FFFFFF; display: flex; flex-direction: column; flex: 1; }
        .room-card .content .room-title { font-size: 1.5rem; font-weight: 500; color: #1A1A1A; margin-bottom: 0.5rem; transition: color 0.3s ease; }
        .room-card:hover .content .room-title { color: #C8A87C; }
        .room-card .content .room-title a { color: inherit; text-decoration: none; }
        .room-card .content .room-description { font-size: 0.85rem; color: #666666; line-height: 1.7; margin-bottom: 1rem; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
        .room-card .content .room-details { display: flex; align-items: center; gap: 1.5rem; padding: 0.75rem 0; border-top: 1px solid #EEEEEE; border-bottom: 1px solid #EEEEEE; margin-bottom: 1.5rem; font-size: 0.8rem; color: #555555; }
        .room-card .content .room-details span { display: flex; align-items: center; gap: 0.3rem; }
        .room-card .content .view-btn { display: inline-block; text-align: center; background: #15232e; color: #FFFFFF; padding: 0.9rem 1.5rem; border: none; font-size: 0.7rem; font-weight: 600; letter-spacing: 0.18em; text-transform: uppercase; cursor: pointer; transition: all 0.3s ease; text-decoration: none; position: relative; overflow: hidden; }
        .room-card .content .view-btn:hover { background: #C8A87C; transform: scale(1.02); box-shadow: 0 8px 30px rgba(200,168,124,0.3); }
        .room-card .content .view-btn::after { content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: linear-gradient(to right, rgba(255,255,255,0) 0%, rgba(255,255,255,0.1) 50%, rgba(255,255,255,0) 100%); transform: rotate(45deg) translateX(-100%); transition: transform 0.6s ease; }
        .room-card .content .view-btn:hover::after { transform: rotate(45deg) translateX(100%); }
        .price-badge { position: absolute; top: 1rem; left: 1rem; background: #C8A87C; color: #FFFFFF; padding: 0.35rem 1rem; font-size: 0.7rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; box-shadow: 0 4px 15px rgba(200,168,124,0.3); }
        /* ─── Responsive: Tablet/Desktop ─── */
        @media (min-width: 768px) {
          .room-card { flex-direction: row; }
          .room-card .image-wrapper { width: 50%; min-height: 320px; }
          .room-card .image-wrapper img { min-height: 320px; }
          .room-card .content { width: 50%; }
        }
        /* ─── Mobile adjustments ─── */
        @media (max-width: 768px) {
          .room-card .content { padding: 1.25rem 1.25rem 1.5rem; }
          .room-card .content .room-title { font-size: 1.25rem; }
          .room-card .content .room-details { flex-wrap: wrap; gap: 0.75rem; }
          .room-card .image-wrapper { min-height: 220px; }
          .room-card .image-wrapper img { min-height: 220px; }
          .filter-grid { grid-template-columns: 1fr !important; gap: 1rem !important; }
          .hero-title { font-size: 2.5rem !important; }
        }
        @media (max-width: 480px) {
          .room-card .image-wrapper { min-height: 180px; }
          .room-card .image-wrapper img { min-height: 180px; }
          .price-badge { font-size: 0.6rem; padding: 0.2rem 0.6rem; }
          .room-number-badge { font-size: 0.7rem; padding: 0.2rem 0.6rem; }
        }
        ...
        <div style={{ background: '#FFFFFF', boxShadow: '0 10px 40px rgba(0,0,0,0.08)', padding: '1.5rem', borderBottom: '2px solid #C8A87C' }}>
          <div className="filter-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1.5rem', alignItems: 'end' }}>
            ...
            <div>
              ...
              <select value={roomTypeFilter} onChange={(e) => setRoomTypeFilter(e.target.value)} style={{ width: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid #E0E0E0', padding: '0.5rem 0', fontSize: '0.9rem', color: '#1A1A1A', outline: 'none', cursor: 'pointer' }}>
            ...
            <div>
              ...
              <select value={capacityFilter} onChange={(e) => setCapacityFilter(e.target.value)} style={{ width: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid #E0E0E0', padding: '0.5rem 0', fontSize: '0.9rem', color: '#1A1A1A', outline: 'none', cursor: 'pointer' }}>
            ...
            <div>
              ...
              <select value={priceFilter} onChange={(e) => setPriceFilter(e.target.value)} style={{ width: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid #E0E0E0', padding: '0.5rem 0', fontSize: '0.9rem', color: '#1A1A1A', outline: 'none', cursor: 'pointer' }}>
            ...
            <div>
              <button onClick={resetFilters} style={{ width: '100%', background: '#15232e', color: '#FFFFFF', padding: '0.75rem 0', border: 'none', fontSize: '11px', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer', transition: 'background 0.3s ease' }}
        ...
        {filteredRooms.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            {filteredRooms.map((room, idx) => {
              ...
              // ─── NEW: SAFE ROOM BOOKING CHECK ───
              const roomId = room.documentId || room.id;
              const isBooked = bookedRoomIds.includes(roomId);

              return (
                <ScrollReveal key={roomId || idx} delay={idx * 80}>
                  <div className="room-card" style={{ position: 'relative' }}>
                    {/* ─── UNAVAILABLE OVERLAY ─── */}
                    {isBooked && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.92)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '1rem', textAlign: 'center', padding: '1rem', zIndex: 20 }}>
                    ...
                    <div className="image-wrapper">
                      {imageUrl ? (
                        <img src={`${STRAPI_URL}${imageUrl}`} alt={roomData?.title || 'Room'} />
                      ) : (
                    ...
                    <div className="content">
                      ...
                      <Link
                        href={`/rooms/${roomId}${checkIn && checkOut ? `?checkIn=${checkIn}&checkOut=${checkOut}` : ''}`}
                        className="view-btn"
                      >
                        View Details
                      </Link>
          ...
          <div style={{ textAlign: 'center', background: '#FFFFFF', border: '1px solid #E8E8E8', padding: '2rem', maxWidth: '448px', margin: '0 auto' }}>
            ...
            <button onClick={resetFilters} style={{ background: '#15232e', color: '#FFFFFF', padding: '0.75rem 1.5rem', border: 'none', fontSize: '11px', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer', transition: 'background 0.3s ease' }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#C8A87C'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = '#15232e'; }}
            >
              Reset Filters
