...
export default function DashboardPage() {
  ...
  const fetchUserAndBookings = async (token: string) => {
    ...
    try {
      ...
      // 2. Fetch bookings filtered by EMAIL (works because 'email' exists)
      //    Use populate=room to get the room relation (but NOT photos)
      ...
      const enrichedBookings = await Promise.all(
        bookingsData.map(async (booking: any) => {
          // Get room documentId from the populated room relation
