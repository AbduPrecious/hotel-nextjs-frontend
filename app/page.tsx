...
import { getHotelDetails, getAvailableRooms, getAmenities, getTestimonials } from './lib/api';
...
export default function Home() {
  ...
  // ─── Infinite Looping Carousel State & Refs ──────────────
  ...
  // ─── Hero Slider State (Manual Prev/Next) ────────────────────
  let mediaList = [];
  ...
  // ─── Carmelina-style Redirect Functionality ────────────────
  ...
  useEffect(() => {
    async function loadData() {
      const [hotelData, roomsData, menuData, amenitiesData, testimonialsData, aboutData] = await Promise.all([
        getHotelDetails(),
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', paddingTop: isMobile ? '56px' : '64px'}}>
      <style>{`
        @keyframes spin {
...
<section style={{ position: 'relative', height: '100vh', minHeight: isMobile ? '500px' : '600px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 1rem', overflow: 'hidden', background: '#FFFFFF' }}>
  ...
  {/* --- Dynamic Media Background (Proper Cross-Fade Animation) --- */}
  {mediaList.length > 0 ? (
    <>
      {/* Loop through ALL slides and fade them based on index */}
      {mediaList.map((media, idx) => (
        <div
          ...
          style={{
            position: 'absolute',
            inset: 0,
            zIndex: idx === mediaIndex ? 1 : 0, // Active slide sits on top
            opacity: idx === mediaIndex ? 1 : 0,
            transition: 'opacity 2.5s ease-in-out', // Smooth 1.2s cross-fade
          }}
        >
          {isVideo(media) ? (
            <video
      ...
      {/* ─── Dark overlay (Stays fixed at zIndex 2) ─── */}
  ...
  <div style={{ position: 'relative', zIndex: 10, maxWidth: '896px', margin: '0 auto', textAlign: 'center', padding: '0 1rem' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
      <a
        ...
        onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.05)'; e.currentTarget.style.boxShadow = '0 10px 40px rgba(200,168,124,0.3)'; }}
        onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = 'none'; }}
      >
        Explore Rooms
        <svg style={{ width: '1rem', height: '1rem' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
      </a>
      <Link
        href="/rooms"
        style={{
      ...
      {/* ===== CARMELINA CHECK-IN / BOOKING SEARCH BAR ===== */}
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(180px, 1fr))', gap: isMobile ? '1rem' : '1.5rem', alignItems: 'end' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <label style={{ fontSize: '0.72rem', letterSpacing: '0.15em', fontWeight: 600, color: '#777777', textTransform: 'uppercase', marginBottom: '0.5rem' }}>
                CHECK - IN
              </label>
              <input
                type="date"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gridColumn: isMobile ? '1 / -1' : 'span 1' }}>
              <button
                ...
                onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#B69B78'; }}
                onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#17232E'; }}
              >
                CHECK AVAILABILITY
              </button>
            </div>
      ...
      {/* ===== ABOUT ===== */}
      {about && (
        <section style={{ padding: isMobile ? '2rem 1rem' : '4rem 1rem', background: '#0A0A0A', borderTop: '1px solid #333333' }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr', gap: isMobile ? '1.5rem' : '3rem', alignItems: 'center' }}>
            <div>
              {about.image?.url ? (
                <img src={`${STRAPI_URL}${about.image.url}`} alt={about.title || 'About us'} style={{ width: '100%', height: isMobile ? '200px' : '300px', objectFit: 'cover', borderRadius: '1rem' }} />
              ) : (
                <div style={{ width: '100%', height: isMobile ? '200px' : '300px', background: '#1A1A1A', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#A0A0A0' }}>No image</div>
              )}
            </div>
            <div>
              <span style={{ color: '#C8A87C', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 600, display: 'inline-block', marginBottom: '0.5rem' }}>About Our Hotel</span>
              <h2 style={{fontSize: isMobile ? '1.8rem' : '2.5rem', fontWeight: 700, color: '#F5F5F5', lineHeight: '1.2' }}>{about.title || 'We Strive Only for the Best!'}</h2>
              <p style={{ color: '#A0A0A0', fontSize: '0.875rem', lineHeight: '1.625', marginTop: '1rem' }}>{about.description || 'Experience the perfect blend of comfort and elegance.'}</p>
              {about.quote && (
                <blockquote style={{ marginTop: '1.5rem', borderLeft: '4px solid #C8A87C', paddingLeft: '1rem' }}>
                  <p style={{ color: 'white', fontStyle: 'italic', fontSize: '0.875rem' }}>{about.quote}</p>
                  <footer style={{ color: '#C8A87C', fontSize: '0.875rem', marginTop: '0.5rem' }}>— {about.quote_author || 'General Manager'}</footer>
                </blockquote>
              )}
            </div>
          </div>
               ...
               {/* ===== CARMELINA COMPACT INFINITE LOOPING ROOMS CAROUSEL ===== */}
<section
  style={{
...
>
    ...
    <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end' }}>
      <Link
        ...
        style={{
          ...
          textDecoration: 'none',
          border: '1px solid #B69B78',
          borderRadius: '9999px',
          padding: isMobile ? '0.4rem 1rem' : '0.5rem 1.25rem',
          transition: 'all 0.3s ease',
          whiteSpace: 'nowrap', // Prevents awkward wrapping on tiny screens
        }}
        onMouseEnter={(e) => {
          e.currentTarget.style.background = '#B69B78';
          e.currentTarget.style.color = '#111111';
          e.currentTarget.style.transform = 'scale(1.05)';
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.background = 'transparent';
          e.currentTarget.style.color = '#B69B78';
          e.currentTarget.style.transform = 'scale(1)';
        }}
      >
        View All Rooms
    ...
    <span
      style={{
        ...
        fontWeight: 600,
        display: 'block',
        marginTop: isMobile ? '-0.25rem' : 0, // Pulls up slightly on mobile for tighter spacing
        marginBottom: '0.5rem',
      }}
    >
      OUR ROOMS
  ...
  <div style={{ width: '100%', overflow: 'hidden', position: 'relative' }}>
    <div
      ...
      style={{
        display: 'flex',
        alignItems: 'center',
        transform: `translateX(calc(50vw - ${CARD_WIDTH / 2}px - ${roomsIndex * ITEM_FULL_WIDTH}px))`,
            ...
            <div
              style={{
                ...
                background: isHovered
                  ...
                  : 'linear-gradient(to top, rgba(23,35,46,0.95) 0%, rgba(23,35,46,0.5) 60%, transparent 100%)',
                padding: isHovered
                  ? isMobile ? '0.8rem 1rem' : '1.2rem 1.5rem 1rem 1.5rem'
                  : isMobile ? '1rem 1rem 0.8rem 1rem' : '2rem 1.5rem 1.5rem 1.5rem',
            ...
            >
              ...
              <h3
                style={{
                  ...
                  fontSize: isHovered ? (isMobile ? '1.1rem' : '1.4rem') : (isMobile ? '1rem' : '1.25rem'),
                  fontWeight: 500,
                  margin: '0 0 0.25rem 0',
                  color: isHovered ? '#17232E' : '#FFFFFF',
              ...
              <div
                style={{
                  ...
                  alignItems: 'center',
                  gap: '0.5rem',
                  fontSize: isHovered ? (isMobile ? '0.7rem' : '0.85rem') : (isMobile ? '0.6rem' : '0.75rem'),
                  color: isHovered ? '#555' : '#D0D5DD',
                  marginBottom: '0.6rem',
                  fontWeight: 300,
                  transition: 'all 0.3s ease',
                  flexWrap: 'wrap',
              ...
              >
                {roomData?.size && <span>{roomData.size} m²</span>}
                <span>•</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                  <svg width={isMobile ? "12" : "14"} height={isMobile ? "12" : "14"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M2 10v7a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-7" />
              ...
              {isHovered ? (
                <Link
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.color = '#17232E';
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.color = '#B69B78';
                  }}
                >
                  ROOM DETAILS
                  <svg width={isMobile ? "14" : "18"} height={isMobile ? "14" : "18"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginLeft: '0.5rem' }}>
                    <path d="M5 12h14" />
                ...
                <Link
                  ...
                  style={{
                    ...
                    borderBottom: '1.5px solid #B69B78',
                    paddingBottom: '0.15rem',
                  }}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.color = '#E8D5B8';
                    e.currentTarget.style.borderColor = '#E8D5B8';
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.color = '#B69B78';
                    e.currentTarget.style.borderColor = '#B69B78';
                  }}
                >
                  ROOM DETAILS
                </Link>
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          alignItems: 'center',
          justifyContent: 'center',
          fontSize: isMobile ? 'inherit' : '1.5rem',
          cursor: 'pointer',
      ...
      {/* ─── Page Numbers ─── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '0.4rem' : '1.25rem', justifyContent: 'center', flexWrap: 'wrap' }}>
        {rooms.map((_, idx) => {
...
<section style={{ padding: isMobile ? '2.5rem 1rem' : '4rem 1rem', background: '#FFFFFF', borderTop: '1px solid #E8E8E8', borderBottom: '1px solid #E8E8E8' }}>
  <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: isMobile ? '1.5rem' : '2rem' }}>
     ...
     {/* ===== AMENITIES ===== */}
<section style={{ padding: isMobile ? '2.5rem 1rem' : '4rem 1rem', background: '#FFFFFF', borderTop: '1px solid #E8E8E8' }} id="amenities">
  <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
    <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
      <span className="section-tag">Facilities</span>
      <h2 className="section-title">Our Amenities</h2>
      <p style={{ color: '#666666', marginTop: '0.5rem', fontSize: '0.875rem', maxWidth: '672px', margin: '0.5rem auto 0' }}>Everything you need for a perfect stay</p>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: isMobile ? '1rem' : '1.5rem' }}>
      {amenities.length > 0 ? (
        amenities
          .slice((amenitiesPage - 1) * PAGE_SIZE, amenitiesPage * PAGE_SIZE)
          .map((amenity, idx) => {
            const imageUrl = amenity?.image?.url ? `${STRAPI_URL}${amenity.image.url}` : null;
            return (
              <ScrollReveal key={amenity.documentId || idx} delay={idx * 80}>
                <div
                  style={{
                    background: '#F8F8F8',
        ...
        <div style={{ textAlign: 'center', padding: '3rem 0', color: '#999', gridColumn: '1 / -1' }}>
    ...
    {amenities.length > PAGE_SIZE && (
      ...
      >
        <button
          onClick={() => setAmenitiesPage((p) => Math.max(1, p - 1))}
          disabled={amenitiesPage === 1}
          style={{
            padding: '0.5rem 0.75rem',
            borderRadius: '9999px',
            transition: 'all 0.3s ease',
            background: amenitiesPage === 1 ? '#E8E8E8' : '#C8A87C',
            color: amenitiesPage === 1 ? '#999999' : '#1A1A1A',
            cursor: amenitiesPage === 1 ? 'not-allowed' : 'pointer',
            border: 'none',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
        ...
        <div style={{ display: 'flex', gap: '0.375rem' }}>
          {Array.from({ length: Math.ceil(amenities.length / PAGE_SIZE) }).map((_, idx) => (
            <button
              key={idx}
              onClick={() => setAmenitiesPage(idx + 1)}
              style={{
                height: '0.375rem',
                borderRadius: '9999px',
                transition: 'all 0.3s ease',
                background: amenitiesPage === idx + 1 ? '#C8A87C' : '#E0E0E0',
                width: amenitiesPage === idx + 1 ? '1.5rem' : '0.375rem',
                border: 'none',
                cursor: 'pointer',
              }}
            />
        ...
        <button
          onClick={() => setAmenitiesPage((p) => Math.min(Math.ceil(amenities.length / PAGE_SIZE), p + 1))}
          disabled={amenitiesPage === Math.ceil(amenities.length / PAGE_SIZE)}
          style={{
            padding: '0.5rem 0.75rem',
            borderRadius: '9999px',
            transition: 'all 0.3s ease',
            background: amenitiesPage === Math.ceil(amenities.length / PAGE_SIZE) ? '#E8E8E8' : '#C8A87C',
            color: amenitiesPage === Math.ceil(amenities.length / PAGE_SIZE) ? '#999999' : '#1A1A1A',
            cursor: amenitiesPage === Math.ceil(amenities.length / PAGE_SIZE) ? 'not-allowed' : 'pointer',
            border: 'none',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
  ...
  <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: isMobile ? '1rem' : '1.5rem' }}>
      {testimonials.length > 0 ? (
        testimonials
          .slice((testimonialsPage - 1) * PAGE_SIZE, testimonialsPage * PAGE_SIZE)
          .map((review, idx) => {
            ...
            return (
              <ScrollReveal key={review.documentId || idx} delay={idx * 80}>
                <div
                  style={{
                    background: '#F8F8F8',
                    border: '1px solid #E8E8E8',
                    ...
                    {[...Array(5)].map((_, i) => (
                      <svg
                        ...
                        style={{
                          width: '1.2rem',
                          height: '1.2rem',
                          color: i < (review.rating || 5) ? '#C8A87C' : '#DDD',
                          fill: i < (review.rating || 5) ? '#C8A87C' : 'none',
                        }}
                        viewBox="0 0 20 20"
                      >
      ...
      >
        <button
          ...
          style={{
            ...
            borderRadius: '9999px',
            transition: 'all 0.3s ease',
            background: testimonialsPage === 1 ? '#E8E8E8' : '#C8A87C',
            color: testimonialsPage === 1 ? '#999999' : '#1A1A1A',
        ...
        <div style={{ display: 'flex', gap: '0.375rem' }}>
          {Array.from({ length: Math.ceil(testimonials.length / PAGE_SIZE) }).map((_, idx) => (
            <button
              key={idx}
              onClick={() => setTestimonialsPage(idx + 1)}
              style={{
                height: '0.375rem',
                borderRadius: '9999px',
                transition: 'all 0.3s ease',
                background: testimonialsPage === idx + 1 ? '#C8A87C' : '#E0E0E0',
                width: testimonialsPage === idx + 1 ? '1.5rem' : '0.375rem',
                border: 'none',
                cursor: 'pointer',
              }}
            />
        ...
        <button
          onClick={() => setTestimonialsPage((p) => Math.min(Math.ceil(testimonials.length / PAGE_SIZE), p + 1))}
          disabled={testimonialsPage === Math.ceil(testimonials.length / PAGE_SIZE)}
          style={{
            padding: '0.5rem 0.75rem',
            borderRadius: '9999px',
            transition: 'all 0.3s ease',
            background: testimonialsPage === Math.ceil(testimonials.length / PAGE_SIZE) ? '#E8E8E8' : '#C8A87C',
            color: testimonialsPage === Math.ceil(testimonials.length / PAGE_SIZE) ? '#999999' : '#1A1A1A',
            cursor: testimonialsPage === Math.ceil(testimonials.length / PAGE_SIZE) ? 'not-allowed' : 'pointer',
            border: 'none',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
